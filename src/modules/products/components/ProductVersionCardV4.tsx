import { FaCirclePause, FaFire, FaPlus, FaTriangleExclamation } from "react-icons/fa6";
...
const ProductVersionCardV4 = ({ data, viewMode = "grid", className, imageLoading }: Props) => {
    ...
    // Se salta el primer tag salvo que solo exista uno; hasta 3 renglones (3º también para "+N")
    const hasMultipleTags = sortedTags.length > 1;
    const candidateTags = hasMultipleTags ? sortedTags.slice(1) : sortedTags;
    const showExtraCount = candidateTags.length > MAX_VISIBLE_TAGS;
    const tagsToShow = showExtraCount ? candidateTags.slice(0, MAX_VISIBLE_TAGS - 1) : candidateTags;
    const extraTagsCount = showExtraCount ? candidateTags.length - tagsToShow.length : 0;
    ...
    /* ── Rating ──
       Comentado en V4 por decisión de diseño. Para restaurar, descomentar el bloque
    ...
    */
    ...
    /* ── Tags readonly (texto crudo, 1 por renglón, hasta 3; el 3º puede ser "+N") ── */
    const tagsBlock = (
        <div className="flex flex-col overflow-hidden h-[30px] sm:h-[36px]">
            {tagsToShow.map((tag) => (
                <span
                    key={tag.id}
                    title={tag.name}
                    className={clsx(
                        "truncate leading-none text-[9px] sm:text-[13px] select-none cursor-default",
            ...
            {extraTagsCount > 0 && (
                <span
                    className={clsx(
                        "truncate leading-none text-[9px] sm:text-[10px] font-semibold select-none cursor-default",
    ...
    /* ── Acciones ── */
    const actionsBlock = isUnavailable ? (
        <div className="w-full pt-1 pb-1">
            <span className={clsx(
                "w-full inline-flex items-center justify-center gap-2 h-9 sm:h-10 rounded-xl font-bold text-xs sm:text-sm border",
                isPaused
                    ? "bg-warning/15 text-warning border-warning/25"
                    : "bg-error/15 text-error border-error/25"
            )}>
                {isPaused ? <FaCirclePause className="shrink-0" /> : <FaTriangleExclamation className="shrink-0" />}
                {isPaused ? "Pausado" : "AGOTADO"}
            </span>
        </div>
    ) : (
        <div className="flex gap-2 items-stretch pt-1 pb-1 w-full">
            {/* Buy now */}
            <div className={clsx(!isList && "flex-1")}>
                <button
                    type="button"
...
export default ProductVersionCardV4;