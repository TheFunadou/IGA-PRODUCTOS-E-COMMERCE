...
import { FaListUl, FaCheckCircle, FaClock, FaMapMarkerAlt } from "react-icons/fa";
...
const PrivacyPolicy = () => {
    document.title = "Iga Productos | Política de Privacidad";
    ...
    const sections = [
        ...
        { id: "location", label: "Solicitud de ubicación" },
            ...
            <div className="w-full md:w-80/100 px-2 sm:px-3 md:px-4 py-6 md:py-10 rounded-2xl">
                ...
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                        <FaShieldHalved className="text-primary text-lg sm:text-xl" />
                    </div>
                ...
                <div className="flex gap-6 xl:gap-8">
                    ...
                    <div ref={contentRef} className="flex-1 min-w-0 flex flex-col gap-8 sm:gap-10 pb-6">
                        ...
                        <div className="scroll-mt-28">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-1 h-8 rounded-full bg-primary shrink-0" />
                                <h2 id="intro" className="text-lg sm:text-xl font-bold text-base-content">Política de privacidad</h2>
                            </div>
                            <div className="bg-base-200 border border-base-300 rounded-2xl px-5 sm:px-8 py-6 sm:py-8">
                                ...
                                <div className="mt-6 flex flex-wrap gap-3">
                                    {[
                                        { label: "Transparencia técnica", icon: <FaCheckCircle className="text-xs" /> },
                                        { label: "Análisis interno", icon: <FaCheckCircle className="text-xs" /> },
                                        { label: "Sin rastreo de IP", icon: <FaCheckCircle className="text-xs" /> },
                                    ].map((badge) => (
                        ...
                        <PolicySection id="cookies" title="Cookies y Persistencia">
                            ...
                            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 not-prose">
                                {[
                                    { type: "Cookies de Sesión", duration: "24 horas", desc: "Mantenimiento del estado técnico", icon: <FaClock className="text-xs" /> },
                                    { type: "LocalStorage (Guest)", duration: "Temporal", desc: "ID de sesión único para compras", icon: <MdTimer className="text-xs" /> },
                                ].map((c) => (
                                    <div key={c.type} className="bg-base-200 rounded-xl px-4 py-3 border border-base-300">
                                        <p className="text-xs font-bold text-primary">{c.type}</p>
                                        <p className="text-[0.70rem] text-base-content/60 mt-0.5">{c.desc}</p>
                                        <p className="text-xs font-semibold text-base-content mt-2 flex items-center gap-1.5">
                                            {c.icon} {c.duration}
                                        </p>
                                    </div>
                                ))}
                        ...
                        <PolicySection id="my-rights" title="Tus derechos sobre los datos">
                            ...
                            <div className="mt-5 flex flex-wrap gap-2 not-prose">
                                {["Actualización de cuenta", "Borrado de sesión", "Control de seguridad"].map((r) => (
                                    <span key={r} className="bg-primary text-white text-xs font-semibold px-3 py-1.5 rounded-lg">
                                        {r}
                                    </span>
                                ))}
                            </div>
                        ...
                        <PolicySection id="location" title="Solicitud de ubicación">
                            <p>
                                Nuestra plataforma puede solicitar acceso a tu ubicación geográfica con el propósito de <strong>determinar el costo de envío</strong> de tu pedido, calculando la distancia desde nuestro almacén hasta tu dirección de entrega.
                            </p>
                            <br />
                            <p>
                                <strong>¿Es obligatorio?</strong> No. El uso de tu ubicación es completamente <strong>opcional</strong>. Puedes habilitarlo o deshabilitarlo en cualquier momento desde las opciones de permisos de ubicación de tu navegador o dispositivo.
                            </p>
                            <br />
                            <p>
                                <strong>¿Qué pasa si no lo proporciono?</strong> Podrás continuar navegando y realizando compras sin ninguna restricción. Sin embargo, si no compartes tu ubicación, será necesario que especifiques la información de envío (dirección completa, código postal, ciudad y estado) en el formulario de envío para que podamos calcular el costo correspondiente. Esto aplica únicamente para <strong>usuarios invitados</strong>; los usuarios registrados ya cuentan con esta información almacenada en su perfil.
                            </p>
                            <br />
                            <p>
                                La información de ubicación, cuando se proporciona, se utiliza únicamente durante la sesión activa para el cálculo del costo de envío y no se almacena de forma permanente en nuestros servidores.
                            </p>
                            ...
                            <div className="mt-5 not-prose">
                                <div className="bg-base-200 rounded-xl px-4 py-3 border border-base-300">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <p className="text-xs font-bold text-base-content">Resumen del permiso</p>
                                    </div>
                                    <ul className="space-y-1.5 text-sm text-base-content/70">
                                        <li className="flex items-start gap-2">
                                            <FaCheckCircle className="text-success text-xs mt-0.5 shrink-0" />
                                            <span>Opcional — puedes rechazarlo sin afectar tu experiencia</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FaCheckCircle className="text-success text-xs mt-0.5 shrink-0" />
                                            <span>Se usa únicamente para calcular el costo de envío</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FaCheckCircle className="text-success text-xs mt-0.5 shrink-0" />
                                            <span>Puedes habilitarlo o deshabilitarlo desde tu navegador</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FaCheckCircle className="text-success text-xs mt-0.5 shrink-0" />
                                            <span>No se almacena permanentemente en nuestros servidores</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <FaCheckCircle className="text-success text-xs mt-0.5 shrink-0" />
                                            <span>Invitados sin ubicación deberán ingresar dirección en el formulario de envío</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
