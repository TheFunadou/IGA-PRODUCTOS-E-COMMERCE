...
const PaymentExiting = () => {
    ...
    useEffect(() => {
        ...
        if (data.status === "APPROVED" && data.order.details.order.uuid === orderStore?.folio) {
            success();
        ...
        return (
            <div className="bg-base-100 rounded-xl p-4 sm:p-6 md:p-10">
                <div className="flex flex-col sm:flex-row items-center gap-2">
                    <h1 className="text-lg sm:text-2xl md:text-3xl text-center sm:text-left">Estamos procesando tu pago, espere unos momentos...</h1>
    ...
    return (
        <div className="w-full animate-fade-in-up">
            <div className="w-full bg-base-300 px-3 sm:px-4 md:px-5 py-6 sm:py-8 md:py-10 rounded-xl">
                <p className="text-xl sm:text-2xl md:text-3xl font-bold bg-success flex items-center gap-2 rounded-xl p-3">
                    <FaCheckCircle className="flex-shrink-0" />
                    <span className="break-words">Pago procesado exitosamente</span>
                </p>
                                            ...
                                            {details.payments_details.map((det, index) => (
                                                <div key={`payment-${index}`} className="w-full flex flex-col sm:flex-row justify-between gap-3 sm:gap-2 border border-gray-300 rounded-xl p-2">
                                                    <div className="w-full sm:w-1/2 flex flex-col gap-3 sm:gap-4">
                                                        <div>
                                                            <h4 className="px-2 py-1 bg-gray-100 rounded-xl text-sm sm:text-base">Terminación</h4>
                                                            <p className="px-2">{det.last_four_digits}</p>
                                                        </div>
                                                        <div>
                                                            <h4 className="px-2 py-1 bg-gray-100 rounded-xl text-sm sm:text-base">Clase de tarjeta</h4>
                                                            <p className="px-2">{formatPaymentClass[det.payment_class]}</p>
                                                        </div>
                                                        <div>
                                                            <h4 className="px-2 py-1 bg-gray-100 rounded-xl text-sm sm:text-base">Tipo de tarjeta</h4>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="w-full bg-base-100 rounded-xl mt-3 p-2">
                                            <div className="w-full flex flex-col gap-2 pb-3 sm:pb-4 md:pb-5">
                                                ...
                                                <div className="text-lg sm:text-xl md:text-2xl font-bold flex">
                                                    <p className="w-3/5">Total a Pagar:</p>
                                                    <p className="pl-2 break-all">${total}</p>
                                                </div>
                                            </div>
                                            <button className="btn btn-primary mt-3 w-full sm:w-auto">Descargar página</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
