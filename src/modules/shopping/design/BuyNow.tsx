...
const BuyNow = () => {
    ...
    // Hook para obtener el item específico (Buy Now)
    const { data, isLoading, error: fetchError, refetch } = useFetchBuyNowItem({
        item: {
            item: { productUUID: productUUID!, sku: sku?.toUpperCase()! },
    ...
    useEffect(() => {
        if (!addresses) return;
        const defaultAddress = addresses.data.find(a => a.defaultAddress);
    ...
    const cardsMap = new Map(data.cards.map(c => [c.sku.toLowerCase(), c]));
    ...
    const handleCreateOrder = async () => {
        ...
        const buyNowItem: ShoppingCartI = {
            item: {
    ...
    const orderSummaryProps: OrderSummaryProps = {
        subtotalBeforeIva: data.resume?.itemsSubtotalBeforeTaxes || "0.00",
        iva: data.resume?.iva || "0.00",
                    ...
                    <div className="w-full rounded-2xl bg-base-100 border border-base-300 overflow-hidden">
                        ...
                        <div className="divide-y divide-base-300">
                            {data.shoppingCart.map((item, index) => {
                                const card = cardsMap.get(item.item.sku.toLowerCase());
                                if (!card) return null;
                                return (
                                    <ShoppingCartItemV2
                                        key={index}
                                        cartItem={item}
                                        cardData={card}
                                        stockLimit={card.stock ?? 0}
                                        onToggleCheck={() => { }}
                                        onRemoveItem={() => { }}
                                        onUpdateQty={(newItem) => handleQuantityChange(newItem.quantity)}
                                        isAuth={isAuth}
                                        isBuyNow={true}
                                    />
                                );
